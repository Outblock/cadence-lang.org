...
1. Provide the reference type `&String` using a type assertion:
...
1. Create another reference to the `String` `hello`.
1. Provide the reference type `&String` using a type annotation instead:
...
1. Create a new instance of the resource type `Counter` and create a reference to it, typed as `&Counter`, so the reference allows access to all fields and functions of the counter:
